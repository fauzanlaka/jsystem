...
<table class="table table-striped table-hover">
    ...
    <?php
        $q = $_POST['q'];
        $sql = mysqli_query($con , "SELECT s.*,p.* FROM students s INNER JOIN pretest p ON s.st_id=p.st_id WHERE (s.firstname_rumi LIKE '%".$q."%' OR s.firstname_jawi LIKE '%".$q."%' OR s.t_studentname LIKE '%".$q."%' OR s.cityzen_id ='$q' OR p.regNumber = '$q') AND payStatus='1' AND type='1'");
